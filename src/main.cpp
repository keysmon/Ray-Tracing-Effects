...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        const Vector4d specular = obj_specular_color*(pow(fmax(0,h.dot(N)),obj_specular_exponent));
    ...
    // use refl_color
        Vector3d r = - 2* N*(N.dot(ray_direction.normalized())) + ray_direction.normalized();
        Vector4d reflection_color = refl_color.cwiseProduct(shoot_ray(p+0.0001*r,r,max_bounce-1));
